...
<head>
    ...
    <title>Create Payment Plan - Land Sales Portal</title>
        ...
        <div class="row align-items-center">
            <div class="col-lg-8">
                ...
                <p class="page-subtitle-modern">
                    Set up a flexible payment plan for your land listing with modern financial tools
                </p>
            </div>
            <div class="col-lg-4 text-end">
                <button type="button" 
                        class="btn btn-secondary-modern"
                        onclick="window.location.href='${pageContext.request.contextPath}/seller/${userId}/payment-plans'; console.log('Back to Plans button clicked');">
...
<div class="container">
    ...
    <div class="row">
        <!-- Left Column - Land Selection & Preview -->
        <div class="col-lg-5 mb-4">
            ...
            <div class="card-modern slide-up">
                ...
                <div class="card-body-modern">
                    <c:if test="${empty userLands}">
                        <div class="alert alert-warning-modern text-center">
                            <i class="fas fa-info-circle fa-2x mb-3"></i>
                            <h5 class="fw-bold">No Available Lands</h5>
                            <p class="mb-3">You need lands with "AVAILABLE" status to create payment plans.</p>
                            <a href="${pageContext.request.contextPath}/lands/post"
                    ...
                    <c:if test="${not empty userLands}">
                        <div class="form-group-modern">
                            <label for="landSelect" class="form-label-modern">
                                Choose Land Property
                            </label>
                            <select class="form-select-modern" id="landSelect" onchange="onLandSelectChange()" required>
                                <option value="">-- Select a land for payment plan --</option>
                                <c:forEach var="land" items="${userLands}">
                                    <option value="${land.id}"
                                        ...
                                        <fmt:formatNumber value="${land.price}" type="currency" currencyCode="LKR"/>
                                    </option>
                                </c:forEach>
                            </select>
                        </div>
                        ...
                        <div class="preview-card-modern d-none" id="landDetailsCard">
                            ...
                            <div class="preview-item-modern">
                                ...
                                <span class="preview-value-modern" id="detailTitle">-</span>
                            </div>
                            <div class="preview-item-modern">
                                <span class="preview-label-modern">Location:</span>
                                <span class="preview-value-modern" id="detailLocation">-</span>
                            </div>
                            <div class="preview-item-modern">
                                <span class="preview-label-modern">Status:</span>
                                <span class="badge-modern badge-success-modern" id="detailStatus">-</span>
                            </div>
                            <div class="preview-item-modern">
                                <span class="preview-label-modern">Price:</span>
                                <span class="preview-value-modern" id="detailPrice">-</span>
                            </div>
                            <div class="preview-item-modern">
                                <span class="preview-label-modern">Size:</span>
                                <span class="preview-value-modern" id="detailSize">-</span>
                            </div>
                        </div>
                    </c:if>
                </div>
            ...
            <div class="preview-card-modern d-none" id="amountPreviewCard">
                <h4 class="preview-title-modern">
                ...
                <div class="preview-item-modern">
                    ...
                    <span class="preview-value-modern" id="previewLandPrice">LKR 0.00</span>
                </div>
                <div class="preview-item-modern">
                    <span class="preview-label-modern">Total Amount:</span>
                    <span class="preview-value-modern" id="previewTotalAmount">LKR 0.00</span>
                </div>
                <div class="preview-item-modern">
                    <span class="preview-label-modern">Down Payment:</span>
                    <span class="preview-value-modern" id="previewDownPayment">LKR 0.00</span>
                </div>
                <div class="preview-item-modern">
                    <span class="preview-label-modern">Remaining Amount:</span>
                    <span class="preview-value-modern" id="previewRemainingAmount">LKR 0.00</span>
                </div>
                <div class="preview-item-modern">
                    <span class="preview-label-modern">Installments:</span>
                    <span class="preview-value-modern" id="previewInstallmentCount">0</span>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-7">
            <div class="card-modern slide-up">
                ...
                <div class="card-body-modern">
                    <form:form method="POST" modelAttribute="paymentPlan" onsubmit="return validateForm()">
                        <input type="hidden" name="landId" id="selectedLandId" required>
                        ...
                        <div class="alert alert-info-modern">
                            ...
                            <div>
                                <strong>Important:</strong> Payment plans require admin approval before becoming active.
                            </div>
                        </div>

                        <!-- Action Buttons -->
                        <div class="d-flex gap-3 justify-content-end">
                            <button type="reset" class="btn btn-secondary-modern" onclick="resetForm()">
...
<script>
    let selectedLandPrice = 0;
    let selectedLandId = null;

    function onLandSelectChange() {
        const landSelect = document.getElementById('landSelect');
        const selectedOption = landSelect.options[landSelect.selectedIndex];
        const landDetailsCard = document.getElementById('landDetailsCard');
        const amountPreviewCard = document.getElementById('amountPreviewCard');
        const submitBtn = document.getElementById('submitBtn');

        if (selectedOption.value) {
            // Get land data from data attributes
            selectedLandId = selectedOption.value;
            selectedLandPrice = parseFloat(selectedOption.getAttribute('data-price'));
            const landTitle = selectedOption.getAttribute('data-title');
            const landLocation = selectedOption.getAttribute('data-location');
            const landSize = selectedOption.getAttribute('data-size');
            const landStatus = selectedOption.getAttribute('data-status');

            // Update land details card
            document.getElementById('detailTitle').textContent = landTitle;
            document.getElementById('detailLocation').textContent = landLocation;
            document.getElementById('detailStatus').textContent = landStatus;
            document.getElementById('detailPrice').textContent = formatCurrency(selectedLandPrice);
            document.getElementById('detailSize').textContent = parseFloat(landSize).toLocaleString('en-US', {
                minimumFractionDigits: 2,
                maximumFractionDigits: 2
            }) + ' sq.ft';

            // Show the cards with animation
            landDetailsCard.classList.remove('d-none');
            amountPreviewCard.classList.remove('d-none');
            landDetailsCard.classList.add('fade-in');
            amountPreviewCard.classList.add('fade-in');

            // Update amount preview
            document.getElementById('previewLandPrice').textContent = formatCurrency(selectedLandPrice);

            // Set hidden input for form submission
            document.getElementById('selectedLandId').value = selectedLandId;
            ...
            // No land selected, hide cards and disable form
            landDetailsCard.classList.add('d-none');
            amountPreviewCard.classList.add('d-none');
            selectedLandId = null;
            selectedLandPrice = 0;
            document.getElementById('selectedLandId').value = '';
    ...
    function enableFormFields(enable) {
        const fields = ['planName', 'totalAmount', 'downPayment', 'installmentCount', 'paymentFrequency', 'interestRate', 'terms'];
    ...
    function resetAmountFields() {
        ...
        // Reset preview
        document.getElementById('previewTotalAmount').textContent = formatCurrency(0);
        document.getElementById('previewDownPayment').textContent = formatCurrency(0);
        document.getElementById('previewRemainingAmount').textContent = formatCurrency(0);
        document.getElementById('previewInstallmentCount').textContent = '0';
        document.getElementById('previewInstallmentAmount').textContent = formatCurrency(0);
    }

    function resetForm() {
        document.getElementById('landSelect').selectedIndex = 0;
        onLandSelectChange();
        document.getElementById('planName').value = '';
        document.getElementById('paymentFrequency').selectedIndex = 0;
    ...
    function validateForm() {
        const landId = document.getElementById('selectedLandId').value;
        if (!landId) {
            Swal.fire({
                icon: 'error',
                title: 'Land Selection Required',
                text: 'Please select a land for the payment plan.',
                confirmButtonColor: '#667eea',
                background: '#fff',
                showClass: { popup: 'animate__animated animate__fadeInDown' },
                hideClass: { popup: 'animate__animated animate__fadeOutUp' }
            });
            document.getElementById('landSelect').focus();
            return false;
        }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initially disable form fields until land is selected
        enableFormFields(false);

        // Auto-focus on land select
        document.getElementById('landSelect').focus();

        // Ensure all buttons are clickable
        const buttons = document.querySelectorAll('.btn-modern, .btn-secondary-modern, .btn-primary-modern');
